...
## 2. データベース構築・データ管理
新規環境でのデータベース構築およびデモデータの管理手順です。

### データベースの構築（テーブル作成・データ投入）
`.env` で `USE_DUMMY_DATA=False` に設定した後、以下のスクリプトを実行します。
このコマンドにより、データベースへの接続、**テーブルの自動作成**（未作成時）、および初期データの投入が一括で行われます。
